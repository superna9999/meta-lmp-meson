...
setenv bootcmd_load_f 'ext4load ${devtype} ${devnum}:2 ${kernel_addr_r} "/boot"${kernel_image}'
setenv bootcmd_run 'bootm ${kernel_addr_r}'
...
run bootcmd_resetvars
ext4load ${devtype} ${devnum}:2 ${scriptaddr} /boot/loader/uEnv.txt
env import -t ${scriptaddr} ${filesize} kernel_image bootargs kernel_image2 bootargs2
